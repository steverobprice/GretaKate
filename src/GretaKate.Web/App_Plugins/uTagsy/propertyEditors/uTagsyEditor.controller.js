...
.controller("uTagsy.PropertyEditors.TagsController",
    ...
    //function ($rootScope, $scope, $log, assetsService) {
        ...
        $http.get("/umbraco/api/uTagsyApi/GetAllTags/?currentNodeId=" + currentNodeId)
        ...
        $scope.initTagNamesFromIds = function () {
            $http.get("/umbraco/api/uTagsyApi/GetTagNames/?nodeIds=" + $scope.model.value)
                 .success(function (data, status, headers, config) {
        ...
        $scope.$on("formSubmitting", function (ev, args) {
            $.ajax({
                url: "/umbraco/api/uTagsyApi/GetAndEnsureNodeIdsForTags/?currentNodeId=" + currentNodeId + "&tags=" + $scope.currentTags,
                async:false
            }).done(function (data) {
